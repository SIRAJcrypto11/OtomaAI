...
import { getMarketplaceTemplates } from "@/app/actions/marketplace";
...
export default async function MarketplacePage({ searchParams }: { searchParams?: { q?: string, category?: string } }) {
    ...
    const templates = await getMarketplaceTemplates(q, category);
    ...
    return (
        <div className="flex-1 p-8 overflow-y-auto bg-slate-50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <form className="relative" action="/dashboard/marketplace" method="GET">
            ...
            <div className="flex gap-2 overflow-x-auto pb-4 mb-6 -mx-1 px-1">
                {["All", "Customer Service", "Sales & Marketing", "Human Resources", "Data & Research", "Free Templates"].map((cat, i) => {
                    ...
                    return (
                        <a
                            key={i}
                            href={`/dashboard/marketplace?category=${encodeURIComponent(cat)}${q ? `&q=${encodeURIComponent(q)}` : ''}`}
                            className={`whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium border ${isActive ? 'bg-slate-900 text-white border-transparent' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                        >
                            {cat}
                        </a>
                    );
                    ...
                    <div key={tpl.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full">
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            ...
                            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                                ...
                                <form action="/dashboard/agents/create">
                                    <button type="submit" className="text-blue-600 bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
